...
var log4javascript = function () {
    ...
    function AjaxAppender(e) {
      function t(e) {
        return I ? (handleError('AjaxAppender: configuration option \'' + e + '\' may not be set after the appender has been initialized'), !1) : !0;
      }
      function n() {
        if (u && enabled) {
          E = !0;
      ...
      var d = this.defaults.timed, p = this.defaults.waitForResponse, h = this.defaults.batchSize, g = this.defaults.timerInterval, f = this.defaults.requestSuccessCallback, m = this.defaults.failCallback, v = this.defaults.postVarName, y = this.defaults.sendAllOnUnload, b = this.defaults.contentType, w = null, C = [], S = [], T = [], E = !1, I = !1;
        ...
        for (var d, p = '', h = this.formatString; d = e.exec(h);) {
          ...
          if (v)
            ...
            case a:
              ...
              break;
            case l:
              var I = T > 0, k = I ? '-' : '+', R = Math.abs(T), A = '' + Math.floor(R / 60);
              A = g(A, 2);
              var x = '' + R % 60;
              x = g(x, 2), p += k + A + x;
            }
